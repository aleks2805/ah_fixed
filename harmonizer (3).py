...
def export_music(score, beat_data, chord_data, filename, repeat_chord=REPEAT_CHORD, outputs_path=OUTPUTS_PATH, water_mark=WATER_MARK):
    ...
    # Если filename уже просто имя (без пути и расширения) — оставить как есть
    if not filename.endswith('.mid') and not filename.endswith('.xml') and not filename.endswith('.mxl') and not filename.endswith('.musicxml'):
    # Уже чистое имя, ничего не делаем
      pass
    else:
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '--input' or sys.argv[i] == '-i':
            ...
            i += 2
        elif sys.argv[i] == '--create-folder' or sys.argv[i] == '-f':
            create_subfolder = True
            i += 1
        elif sys.argv[i] == '--name' or sys.argv[i] == '-n':
            output_name = sys.argv[i+1]
            i += 2
        else:
            print(f"Unknown argument: {sys.argv[i]}")
            print("Usage: python harmonizer.py [-i input_dir] [-o output_dir] [-f] [-n output_name]")
            sys.exit(1)
    
    # Create timestamped subfolder if requested
    if create_subfolder:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_dir = os.path.join(output_dir, f"harmonized_{timestamp}")
    
    print(f"Input directory: {input_dir}")
    print(f"Output directory: {output_dir}")
    if output_name:
        print(f"Output filename: {output_name}.mid")

    # Load data from 'inputs'
    filenames = get_filenames(input_dir=input_dir)
    ...
    for idx in trange(len(data_corpus)):
      ...
      chord_data = generate_chord(model, melody_data, beat_data, key_data)
        ...
        # Export music file
      export_filename = output_name if output_name else filename
        
        # Создаём подпапку с именем файла (без расширения)
      file_basename = os.path.basename(filename)
      file_name_no_ext = '.'.join(file_basename.split('.')[:-1])
      file_output_dir = os.path.join(output_dir, file_name_no_ext)
        
      export_music(score, beat_data, chord_data, export_filename, outputs_path=file_output_dir)