...
class DataGenerator(Sequence):
    ...
    def on_epoch_end(self):
        if self.shuffle:
            # При необходимости здесь можно реализовать перемешивание индексов
            pass
...
def build_model(segment_length, rnn_size, num_layers, dropout, weights_path=None, chord_types_path=CHORD_TYPES_PATH, training=True):
    ...
    input_condition_left = Input(shape=(segment_length, 21), name='input_condition_left')
    condition_left = TimeDistributed(Dense(5, activation='relu'))(input_condition_left)

    input_condition_right = Input(shape=(segment_length, 21), name='input_condition_right')
    condition_right = TimeDistributed(Dense(5, activation='relu'))(input_condition_right)

    input_chord_left = Input(shape=(segment_length, len(chord_types)), name='input_chord_left')
    ...
    for idx in range(num_layers):
        ...
        melody_left = LSTM(rnn_size, name='melody_left_'+str(idx+1), return_sequences=return_sequences)(melody_left)
        if idx != num_layers - 1:
        ...
        melody_right = LSTM(rnn_size, name='melody_right_'+str(idx+1), return_sequences=return_sequences)(melody_right)
        if idx != num_layers - 1:
        ...
        condition_left = LSTM(rnn_size, name='condition_left_'+str(idx+1), return_sequences=return_sequences)(condition_left)
        if idx != num_layers - 1:
        ...
        condition_right = LSTM(rnn_size, name='condition_right_'+str(idx+1), return_sequences=return_sequences)(condition_right)
        if idx != num_layers - 1:
        ...
        chord_left = LSTM(rnn_size, name='chord_left_'+str(idx+1), return_sequences=return_sequences)(chord_left)
        if idx != num_layers - 1:
    ...
    merge = concatenate([melody_left, melody_right, condition_left, condition_right, chord_left])        
    ...
    output_chord = Dense(units=len(chord_types), activation='softmax', name='output_chord')(merge)
    
    model = Model(
        inputs=[input_melody_left, input_melody_right, input_condition_left, input_condition_right, input_chord_left],
